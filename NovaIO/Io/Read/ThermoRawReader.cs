...
namespace Nova.Io.Read
{
  public class ThermoRawReader : ISpectrumFileReader
  {
    ...
    private void ProcessSpectrumInformation(IScanFilter? scanFilter = null, ScanStatistics? scanStatistics = null, bool ext = false)
    {
      ...
      {
        //The old way data was processed.
    ...
    /// <summary>
    /// Processes the Trailer Extra Information attached to the Scan Header. This is used when trailer data is contained in a
    /// list relating to each parent ion when there is more than one.
    /// </summary>
      ...
      for (int i = 0; i < trailerData.Length; i++)
      {
        ...
        switch (MetaDictionary.FindMeta(trailerData.Labels[i]))
        {
          ...
          case MetaClass.FaimsCV:
            if (ext) spectrumEx.FaimsCV = Convert.ToDouble(trailerData.Values[i], CultureInfo.InvariantCulture);
            else spectrum.FaimsCV = Convert.ToDouble(trailerData.Values[i], CultureInfo.InvariantCulture);
            break;
          case MetaClass.FaimsState:
            ...
            break;
          case MetaClass.IIT:
            if (ext) spectrumEx.IonInjectionTime = Convert.ToDouble(trailerData.Values[i], CultureInfo.InvariantCulture);
          ...
          case MetaClass.MonoisotopicMZ:
            if (ext)
            {
              if (spectrumEx.Precursors.Count > 0) spectrumEx.Precursors[0].MonoisotopicMz = Convert.ToDouble(trailerData.Values[i], CultureInfo.InvariantCulture);
            }
            else
            {
              if (spectrum.Precursors.Count > 0) spectrum.Precursors[0].MonoisotopicMz = Convert.ToDouble(trailerData.Values[i], CultureInfo.InvariantCulture);
            }
            break;
