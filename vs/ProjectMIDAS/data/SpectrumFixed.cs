...
namespace ProjectMIDAS.Data.SpectrumFixed
{
    public struct sCentroid
    {
        public double Mz { get; set; }
        public double Intensity { get; set; }
        public sCentroid(double mz = 0, double intensity = 0) 
        { 
            Mz = mz;
            Intensity = intensity;
        }
    ...
    /// </summary>
    public class SpectrumFixed : IDisposable
    {
        /// <summary>
        /// Constructor for Spectrum class.
        /// </summary>
        public SpectrumFixed(int sz)
        {
            Centroids = new sCentroid[sz];
        }
        ...
        /// </summary>
        public sCentroid[] Centroids { get; set; }
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!disposedValue)
            {
                if (disposing)
                {
                    //Two options for cleanup are
                    //1. Do nothing and let the garbage collection do the work
                    //Centroids.Clear();
                    //Centroids = null;

                    //2. Reallocate the array to a single value, and see if you recover the memory prior to garbage collection
                    //Centroids = new sCentroid[1];
                }
                disposedValue = true;
            }
        }
